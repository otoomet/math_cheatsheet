...
\int_0^1 t^{a-1} (1- t)^{b-1} \,\dif t
\qquad
a > 0, b > 0.
\end{equation}

\subparagraph{Incomplete beta function}
\begin{equation}
  \label{eq:incomplete-beta}
  B(x; a, b) = 
  \int_0^x t^{a-1} (1- t)^{b-1} \,\dif t.
\end{equation}

\subparagraph{Regularized incomplete beta function}
\label{sec:regularized-incomplete-beta}
\begin{equation}
  \label{eq:regularized-incomplete-beta}
  I_{x}(a, b) =
  \frac{B(x; a, b)}{B(a, b)}
\end{equation}
...
\begin{align}
  ...
  % Normal entropy
  \int_{-\infty}^{\infty}
  \log\left[ \frac{1}{\sigma} \phi(u) \right]
  \frac{1}{\sigma}\phi(u)\,\dif u 
  &=
    \frac{1}{2} \log 2\pi + \log \sigma + \frac{1}{2}
  \\
  & \qquad \text{normal entropy}
  \nonumber\\
  &\\
  % 
...
\index{De Morgain's laws|textbf}
\begin{align}
...
\subsubsection{Simple sequences}
\label{sec:simple-sequences}

\paragraph{Arithmetic sequence}

\begin{equation}
  \label{eq:seq-sum-arith-12}
  \sum_{i = N/2 + 1}^{N} i =
  \frac{1}{8}(3N^{2} + 2N)
  \qquad
  \text{if $N$ even}
...
\paragraph{Sum of unequal numbers}
\begin{multline}
  \label{eq:seq-sum-unequal}
  \sum_{i \not= j}^{N} ij
  =
  \sum_{i, j = 1}^{N} ij - \sum_{i = 1}^{N} i^{2} =
  \left[
    \frac{1}{2} N (N + 1)
  \right]^{2}
  -
...
\end{equation}
If the sequence is given in the form:
$S'=q+q^2+q^3+...$
then use the fact that $S'=qS$.

\selectlanguage{estonian}
Oluline erijuht kui $q=\frac{1 }{
1+r}$:
\begin{equation}
...
\begin{align}
  \label{eq:sum-powers-2}
  1 + 2 + 2^{2} + 2^{3} + \dots + 2^{N}
  &= 2^{N + 1} - 1
  \\
  1 + 2^{2} + 2^{4} + 2^{6} + \dots + 2^{2N}
  &= \frac{1}{3}(2^{2N + 2} - 1)
  \\
  1 + K + K^{2} + K^{3} + \dots + 2^{N}
  &= \frac{1}{K - 1}(K^{N + 1} - 1)
\end{align}
...
Let $X_{0}$, $X_{1}$, $X_{2}$ be \iid distributed with expected value
0.  Consider sample means
\begin{equation}
  \label{eq:mean-difference-for-var}
  \bar X_{k} = \frac{1}{n_{k}} \sum_{i = 1}^{n_{0}} X_{ki}
\end{equation}
Now
\begin{equation}
  \label{eq:var-mean-difference}
  \var (\bar X_{k} - \bar X_{0}) =
  \frac{1}{n_{2}} + \frac{1}{n_{0}}
\end{equation}
and
\begin{equation}
  \label{eq:corr-mean-difference}
  \corr (\bar X_{k} - \bar X_{0}) =
  \frac{\frac{1}{n_{0}}}{
    \sqrt{
      \left(\frac{1}{n_{1}} + \frac{1}{n_{0}} \right)
      \left(\frac{1}{n_{2}} + \frac{1}{n_{0}} \right)
    }
  }
\end{equation}
...
See Figure~\ref{fig:bernoulli-kl-divergence} for KL-devergence between
two Bernoulli distributions.

\begin{figure}
  <<echo = FALSE>>=
  bernoulliKL <- function(p, q) {
     p*log(p/q) + (1 - p)*log((1 - p)/(1 - q))
  }
  n <- 42
  nLevel <- 6
  eps <- 0.02
  qs <- ps <- seq(eps, 1 - eps, length.out = n)
  pq <- plot3D::mesh(ps, qs)
  KLpq <- bernoulliKL(pq$x, pq$y)
  KLqp <- bernoulliKL(pq$y, pq$x)
  par(mfrow = c(1, 2),
      mar = c(3, 3, 1.5, 0) + 0.1,
      mgp = c(2,1,0)
      )
  levels <- c(0.03, 0.1, 0.3, 0.6, 1)
  image(KLpq, col = terrain.colors(30),
        main = "KL(p || q)", xlab = "p", ylab = "q")
  contour(ps, qs, KLpq, levels = levels,
          add = TRUE)
  image(KLqp, col = terrain.colors(30),
        main = "KL(q || p)", xlab = "p", ylab = "q")
  contour(ps, qs, KLqp, levels = levels,
          add = TRUE)
  @
  \caption{
    KL-devergence between Bernoulli($p$) and Bernoulli($q$).
  }
  \label{fig:bernoulli-kl-divergence}
\end{figure}
...
\paragraph{Distribution function and uniform RV: a special case}

Let $U$ be a standard uniform
\index{distributions!uniform}
RV.  Pick an increasing
semi-monotonic function $F:\Real \to [0,1]$ and let $X = F^{-1}(U)$.
Then $\Pr(X < x) = F(x)$.  This means you can create a RV of any
distribution by applying its distribution function inverse to a
standard uniform.
...
Let $X \distas B(p)$ and $Y \distas B(q)$.  Now
\begin{equation}
  \label{eq:uniform-x-gt-y}
  \Pr(X > Y) = p(1 - q)
\end{equation}
...
f(x) &= \frac{1}{B(a,b)} x^{a-1} (1-x)^{b-1}
  ...
  F(x) &= I_{x}(a, b) =
         \frac{B(x; a, b)}{B(a, b)}
         \qquad
         \text{\hyperref[sec:regularized-incomplete-beta]{Regularized incomplete beta function}}
  \\
\text{mode}\qquad
m &= \frac{a-1}{a + b - 2}
\end{align}
...
See Figure~\ref{fig:beta-density-cdf} for example densities/cdf-s.
...
\begin{figure}[ht]
  ...
  matplot(x, y,
          ...
          main = "Density")
  ## cdf
  y <- sapply(ab, function(alpha) pbeta(x, alpha[1], alpha[2]))
  matplot(x, y,
          type="l", lty=1, lwd=2, ylab="", col=pal,
          main = "cdf")
  legend("topleft",
         ...
         lty=1, lwd=2, cex = 0.85, col=pal, bty="n")
  @
  \caption{
    Beta densities (left) and cdf-s (right).
  }
  \label{fig:beta-density-cdf}
\end{figure}

\paragraph{Method of moments estimator}

Let $\mu$ be the sample average and $\sigma^{2}$ its variance.  The MM
estimator for $a$ and $b$ is
\begin{equation}
  \label{eq:beta-method-moments}
  \hat a =
  \frac{\mu^{2}(1 - \mu)}{\sigma^{2}} - \mu
  \qquad
  \hat b =
  \frac{\mu(1 - \mu)^{2}}{\sigma^{2}} - (1 - \mu)
\end{equation}
...
\index{entropy}
\begin{equation}
  \label{eq:normal-entropy}
  H(X) =
  \frac{1}{2}(2\pi \me \sigma^{2}) =
  \frac{1}{2} \log 2\pi + \log \sigma + 1
\end{equation}
...
\begin{align}
  ...
  \lambda\left( \frac{\mu_{X} - \mu_{Y}}
    ...
    \right).
\end{align}
Proof: write $\E[X|X < Y] = \E[X|Z < 0]$ where $Z = X-Y$.  The result
follows from \eqref{eq:normal2d_E[X1|X2<a]}

\paragraph{Other expectations}

Let $X \distas N(\mu, \sigma^{2})$.
\begin{align}
  \label{eq:normal-ex-a2}
  \E (X - \alpha)^{2} = (\mu - \alpha)^{2} + \sigma^{2}
\end{align}

\paragraph{Other properties}

\hyperref[sec:kullback-leibler-divergence]{Kullback-Leibler
  divergence}
between normals $p$ $N(\mu_{1}, \sigma_{1}^{2})$ and
$q$ $N(\mu_{2}, \sigma_{2}^{2})$ is
\begin{equation}
  \label{eq:kl-devergence-normals}
  KL(p\, ||\, q) =
...
\begin{center}
  \begin{tabular}{lcc}
    ...
    \\[1.2ex]
    $\E X^{2}$ & $\frac{1}{6}$ & $\frac{1}{2}$
    \\[1.2ex]
    $\var X$ & $\frac{1}{18}$ & $\frac{1}{18}$
    \\
    \bottomrule
  \end{tabular}
\end{center}
...
\hyperref[sec:80-20-rule]{Pareto rule}:
\index{pareto rule}
upper $(3 - \sqrt{5})/2 \approx 0.382$ of cases possess
...
\paragraph{Minima and maxima}

Let $U_{i} \distas \mathit{Unif}(0,1)$.

$U_{K}^{\mathit{min}} = \min_{i\in\{1\dots K\}} U_{i}$
and
$U_{K}^{\mathit{max}} = \max_{i\in\{1\dots K\}} U_{i}$
are distributed as
\begin{align}
  \label{eq:minmax-u-distribution}
  F^{\mathit{min}}(u) &= 1 - (1 - u)^{K},
  \qquad
                        f^{\mathit{min}}(u) = K(1 - u)^{K - 1}
  \\
  F^{\mathit{max}}(u) & = u^{K},
  \qquad
  f^{\mathit{max}}(u) = K u^{K - 1}
\end{align}
$U_{K}^{\mathit{max}}$ has the same distribution as $U^{1/K}$.
...
\end{equation}
where $\vec\mu$ is the vector of means and $\mat\Sigma$
is the covariance matrix, has density
\begin{equation}
f_{\vec{X}} (\vec x) = 
  (2\mpi)^{-\frac{1}{2} n} 
  |\mat\Sigma|^{-\frac{1}{2}}
...
Entropy:
\begin{equation}
  \label{eq:multivariate-normal-entropy}
  \entropy(\vec{X}) =
  \frac{1}{2} \log | 2\pi \me \mat{\Sigma} |
\end{equation}
...
\subsubsection{Multivariate Uniform}
\label{sec:mv-uniform}

\index{distributions!multivariate uniform|textbf}

Let $X, Y \distas[\iid] \mathit{Unif}(0,1)$.  Then
\begin{align}
  \label{eq:mv-uniform-Pxy}
  \Pr(Y < y | Y < X) &= 2y - y^{2} \\
  \Pr(X < x, Y < y, Y < X) &= \frac{1}{2}y(2x - y)\\
  \Pr(X < x, Y < y | Y < X) &= y(2x - y)
\end{align}
...
\index{distributions!Wishart|textbf}
is a generalization of \hyperref[sec:gamma-distribution]{gamma distribution} to
...
\index{random walk|textbf}
Definition: random walk is a stochastic process
\begin{equation}
  \label{eq:random-walk-general}
  z_{t+1} = z_t + \varepsilon_{t+1}.
\end{equation}


\subsubsection{Discrete random walk}
\label{sec:discrete-random-walk}
...
\begin{equation}
  ...
  \epsilon =
  \begin{cases}
    -1 \quad\text{with probability}\quad q\\
    1 \quad\text{with probability}\quad p = 1 - q.\\
  \end{cases}
\end{equation}

If $z_{0} = 0$, the probability that $z_{t}$ gets to $S$ before
it getting $-S$ is
\begin{equation}
  \label{eq:disrete-rw-hits-S}
  \frac{p^{S}}{p^{S} + q^{S}}.
\end{equation}
If $z_{0} = s$ where $-S < s < S$, then the probability is
\begin{equation}
  \label{eq:discrete-rw-hits-S-asymmetric}
  \frac{p^{S} \big[p^{S} - q^{S}
    \big(\displaystyle\frac{q}{p}\big)^{s}
    \big]}{
    p^{2S} - q^{2S}
  }
\end{equation}
