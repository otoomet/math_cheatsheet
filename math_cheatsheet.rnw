...
\begin{align}
  ...
  \pderiv[S(x)_{i}]{x_{j}} &=
                             \begin{cases}
                               -S(x)_{i} S(x)_{j} & \text{if}\quad
                               i\not=j\\
                               S(x)_{i}(1 -  S(x)_{j}) & \text{if}\quad
                               i=j\\
                             \end{cases}
  \\
  \pderiv[\log S(x)_{i}]{x_{j}} &=
                             \begin{cases}
                               -S(x)_{j} & \text{if}\quad
                               i\not=j\\
...
Let $\mat{A}$ be a $N\times K$ matrix and  $\vec{1}_{N}$ is a
$N\times1$ vector of ones.  Now
\begin{equation}
  \label{eq:diff-by-matrix-1}
  \pderiv[\mat{A}\cdot \vec{x}]{\mat{A}}  
  =
  \vec{1}_{N} \cdot \vec{x}^{T}
\end{equation}

Let $\mat{A}$ be a $N\times M$ matrix $\mat{B}$ $M\times K$, and
$\vec{x}$ $K\times 1$ vector.
\begin{equation}
  \label{eq:diff-by-matix-compound}
  \pderiv[\mat{A}\cdot\mat{B}\cdot \vec{x}]{\mat{B}}  
  =
  \mat{A}^{T}\vec{1}_{N} \cdot \vec{x}^{T}
\end{equation}
