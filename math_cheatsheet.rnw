...
\qquad\text{and}\qquad
  ...
  = \delta_{kj} \vec{x}^{\transpose} \mat{A} +
  x_{j} \vec{a}_{k}^{\transpose}
\end{equation}
where $\vec{0}$ is a vector of zeros of suitable length and
$\delta_{kj}$ is Kroenecker symbol. \index{Kroenecker symbol}
...
\begin{equation}
  ...
  \end{pmatrix}
  +
  (\vec{1}_{N} \otimes \vec{a}_{k}) \odot
  (\vec{x} \otimes \vec{1}_{M}^{\transpose})
\end{equation}
where $\otimes$ is Kronecker product, $\odot$ is element-wise product,
and the the first matrix contains zeros but at the $k$-th row.
