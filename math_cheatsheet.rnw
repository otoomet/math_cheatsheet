...
\usepackage[colorlinks, linkcolor=blue, citecolor=TealBlue]{hyperref}
...
par(mar=c(3,3,0,0)+0.1,
    mgp=c(2,1,0))
curve(asinh, -10, 10, n=201, col="red")
@ 
...
For $K$-dimensional input vector $\vec{x}$, the $c$-th component of softmax is
\begin{equation}
  \label{eq:softmax}
  S(\vec{x})_{c} = \frac{\me^{x_{c}}}{\sum_{k} \me^{x_{k}}}.
\end{equation}
This describes discrete probability distribution between $K$ states
with corresponding weight $\me^{x_{k}}$.  In case of $S(\vec{x}/T)$,
...
m &= \frac{a-1}{a + b - 2}
\end{align}
where $B(a,b)$ is is the \hyperref[sec:beta-function]{beta function} \eqref{eq:beta-fcn}.
...
curve(dgamma(x, 0, 0), 0, 2, col="red", add=TRUE)
legend("topright", 
       legend=c("Ga(0.5, 0.5)", "Ga(0,0)"), 
...
\begin{equation}
  ...
  p(x|\vec{\eta}) = h(x) \me^{\vec{\eta}` T(x) - A(\vec{\eta})},
\end{equation}
where $\eta$ is the canonical parameter, $T(x)$ sufficient statistic,
and $A(\eta)$ cumulant function.  Instead of the canonical parameter, one
