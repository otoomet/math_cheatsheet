...
\dots, a_{iM})^{\transpose}$ is it's $i$-th row, and $\vec{x}$ and $\vec{y}$ be a length-$N$
vector 
$\vec{x} = (x_{1}, x_{2}, \dots, x_{N})^{\transpose}$ 
and
$\vec{y} = (y_{1}, y_{2}, \dots, y_{N})^{\transpose}$.  Further, denote by $\vec{a}_{i\bullet}$ and $\vec{a}_{\bullet j}$ the $i$-th row and 
$j$-th column of matrix $\mat{A}$, both as $N\times1$ column vectors.
Now
\begin{equation}
...
\begin{align}
  ...
  &=
  \vec{x}^{\transpose} \kronProd ( \mat{A} \cdot \vec{y} ) +
  \vec{y}^{\transpose} \kronProd ( \mat{A} \cdot \vec{x} )
\end{align}
...
\index{beta distribution|textbf}
\begin{align}
\text{pdf}\qquad
...
Properties: $B(1,1)$ is uniform distribution.  Larger parameter values
