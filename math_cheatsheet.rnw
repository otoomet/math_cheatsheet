...
\dots, a_{iM})^{\transpose}$ is it's $i$-th row, and $\vec{x}$ be a length-$M$
vector.  Now
\begin{equation}
  \pderiv{a_{k}^{\transpose}} \mat{A} \vec{a_{k}}
  = \mat{A} + 
  \begin{pmatrix}
    \vec{0}^{\transpose}\\
    \vec{0}^{\transpose}\\
  ...
  \end{pmatrix}
\qquad\text{and}\qquad
  \pderiv{a_{k}^{\transpose}} \vec{x}^{\transpose} \mat{A} \vec{a_{k}}
  = \vec{x}^{\transpose} \mat{A} +
  \vec{x}_{k} \vec{a}_{k}^{\transpose}
\end{equation}
where $\vec{0}$ is a vector of zeros of suitable length.
