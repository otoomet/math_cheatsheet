...
\index{incomplete gamma function|textbf}
...
scalars and $\vec{x}$ is a vector.  The chain rule for the first derivative:
\index{chain rule}
\begin{equation}
...
Sum of $k$ squared independent normals is $\chi_{k}^2$-distributed.
Properties: 
\begin{align}
\text{pdf}
\qquad &
...
\item if $X \sim \chi_{1}^{2}$ then
  \begin{equation}
    ...
    \Pr(X < \frac{1}{2}\theta | X < \theta)
    =
    \frac{\gamma(\frac{1}{2}, \frac{1}{4}\theta)}
  ...
  This converges to $1/\sqrt{2}$ if $\theta \to 0$.
\end{itemize}
...
\begin{align}
  ...
  \E X &=
         ...
         \quad \alpha > 1
  \\
  \E X|X > x_{*} &= \frac{\alpha}{\alpha - 1} x_{*}, 
                   \quad\alpha > 1
\end{align}
Moments:
\begin{center}
