...
Let $Y$ be a mixture of $\mathit{Bernoulli(p_{1})}$ with probability
$\alpha$ and $\mathit{Bernoulli(p_{2})}$ with probability $1-\alpha$.  Properties:
\begin{align*}
  \E X &= N[\alpha p_{1} + (1 - \alpha) p_{2}] \equiv N\mu
  \\[1.1ex]
  \var X &= N[\mu - N\mu^{2} +
           (N - 1)(\alpha p_{1}^{2} + (1-\alpha)p_{2}^{2})] =
  \\[1.1ex]
       &= N \mu (1 - \mu) + N (N-1) \alpha(1-\alpha) (p_{1} - p_{2})^{2}
\end{align*}
Note that if $N=1$, $\var X$ is equal to the variance of 
$\mathit{Bernoulli(\mu)}$ RV.  This is not the case if $N > 1$ where
the variance is larger than the variance of a binomial RV with the
average probability, $N\mu(1-\mu)$.
...
the values can start from 0.  It is a shift of
\hyperref[sec:pareto-distribution]{Pareto distribution}.
Let $x_{0}$ be scale.  Properties:
\begin{align}
f_{X}(x) &=
\frac{\alpha}{x_{0}} 
           \left(\frac{x}{x_{0}} + 1 \right)^{-\alpha-1}
\\
\E X &=
\frac{1}{\alpha - 1} x_{0},
\quad \alpha > 1
