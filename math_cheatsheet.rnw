...
\index{Bessel function!modified|textbf}
...
\hyperref[sec:gradient-of-scalar-function]{gradient} \index{gradient} in denominator layout.
...
Let $y = y(\vec{x})$ and $\vec{x} = \vec{x}(z)$ where $y$ and $z$ are
scalars and $\vec{x}$ is a factors.  The chain rule for the first derivative:
\index{chain rule}
\begin{equation}
  \label{eq:matrix-calculus-chain-rule-1}
  \pderiv[y(\vec{x}(z))]{z} 
  =
  \pderiv[y(\vec{x})]{\vec{x}^{\transpose}}\cdot
  \pderiv[\vec{x}(z)]{z}
  =
...
\begin{alignat}{2}
  ...
  \\
  % 
  \frac{\partial \vec{x}^{\transpose}\vec{x}}{\partial \vec{x}} &= \mat{I}\vec{x} + \vec{x}\mat{I} = 2\vec{x} 
  & \qquad
...
\index{80-20 rule|textbf}

This is a rule describing inequality: the upper 20\% of cases posess
80\% of resources.  More generally, define $x^{*}$ through equation
\begin{equation}
...
par(mar=c(3,3,0,0)+0.1,
    mgp=c(2,1,0))
ab <- list(c(1,1), c(0.5, 0.5), c(2, 2), c(0.7, 1.5), c(1.7, 2.5))
...
legend("topright",
       legend=sapply(ab, function(alpha) paste0("B(", alpha[1], ",", alpha[2], ")")),
       lty=1, col=pal, bty="n")
@ 
...
p <- ggplot(data.frame(x=c(0,3))) +
   labs(y = "Density", color=expression(sigma)) +
   geom_function(aes(x, col=as.character(sigmas[1])),
                 fun=dlnorm, args=list(meanlog = mus[1], sdlog=sigmas[1])) +
   geom_function(aes(x, col=as.character(sigmas[2])),
                 fun=dlnorm, args=list(meanlog = mus[2], sdlog=sigmas[2])) +
   geom_function(aes(x, col=as.character(sigmas[3])),
                 fun=dlnorm, args=list(meanlog = mus[3], sdlog=sigmas[3])) +
   geom_function(aes(x, col=as.character(sigmas[4])),
...
\hyperref[sec:pareto-distribution]{Pareto Distribution} \index{distributions!Pareto}.
...
\item it is \emph{scale-free}: there is no features in the right tail,
  wherever you look, you have most observations that are smaller, but
  you also have observations that are way larger.
\item \hyperref[sec:80-20-rule]{80-20 rule}: 
  \index{80-20 rule}
  The critical $x$ threshold $x_{*}$
...
x^alpha - x - 1
solve8020 <- function(alpha) {
  x <- uniroot(criticalEq, c(1, 10000), alpha=alpha)$root
  x^(-alpha) - 0.2
}
xCr <- function(alpha)
