...
How good is the Taylor-series based approximation?
...
sll <- foreach(R = Rs, .combine=rbind) %do% {
   rRes <- foreach(lambda = lambdas, .combine = rbind) %dopar% {
      ...
      simLogL <- sum(log(sPr[1 + x] + kappa))
      pp <- dpois(x, lambda)
      T2 <- sum(dpois(x, lambda, log=TRUE) - 1/2/pp^2 * pp*(1 - pp)/R)
      T3 <- T2 + sum(1/3/pp^3 * pp*(1 - pp)*(1 - 2*pp)/R^2)
      T4 <- T3 - sum(1/4/pp^4 * pp*(1 - pp)*(1 - 3*pp + 3*pp^2)/R^3)
      data.frame(simLogL=simLogL, T2=T2, T3=T3, T4=T4, trueLogL = sum(dpois(x, lambda, log=TRUE)))
   }
   cbind(lambda=lambdas, R=R, rRes)
} 
yl <- range(sll$trueLogL, sll$simLogL, max(sll$T2), min(sll$T3))
sll %<>%
   tidyr::gather(key="type", value="LL", c("simLogL", "T2", "T3", "T4", "trueLogL")) %>%
   mutate(R = factor(R, levels=c(Rs)))
p1 <- ggplot(sll %>% filter(R == Rs[1]), aes(lambda, LL)) +
   geom_line(aes(col=type)) +
   geom_vline(xintercept=mean(x), linetype="dotted") + 
   coord_cartesian(ylim=yl) +
   labs(title=Rs[1])
p2 <- ggplot(sll %>% filter(R == Rs[2]), aes(lambda, LL)) +
   geom_line(aes(col=type)) +
   geom_vline(xintercept=mean(x), linetype="dotted") + 
   coord_cartesian(ylim=yl) +
...
It appears that 2-term approximation is better than the true
likelihood (1-term approximation).  The approximation works better for
larger $R$ values.
